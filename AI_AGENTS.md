...
- [ ] Submit the sitemap (`https://drdebug.dev/sitemap-index.xml`, generated by `@astrojs/sitemap`) inside GSC.
...
## Error-fix library (programmatic SEO)

The `/fix/` section is the programmatic-SEO library the Content strategy
section forecasts: one indexed page per resolved error signature, generated
from a typed content collection. Templating + pipeline only — **content is
filled in by humans and gated by a `verified` flag** (see below). The whole
point is that pages can't be invented or shipped unverified by accident.

### Route structure

```
/fix/                       src/pages/fix/index.astro            Library index: all errors grouped
                                                                 by platform, client-side search +
                                                                 platform filter (ErrorFilter island).
/fix/[platform]/            src/pages/fix/[platform]/index.astro Per-platform index (one static page
                                                                 per platform: lindy / n8n / relay).
/fix/[platform]/[slug]/     src/pages/fix/[platform]/[slug].astro One fix page per collection entry.
                                                                 NB: file is [slug].astro, not
                                                                 [error-slug] — Astro param names
                                                                 forbid hyphens; only the value may
                                                                 contain them.
```

Shared shell: `src/layouts/FixLayout.astro` (header/footer + `SeoHead.astro`
for per-page meta + optional `noindex`). Platform labels/badges and frequency
ordering live in `src/lib/platforms.ts` (the closed platform set — extend the
enum there AND in `src/content.config.ts` to add a platform). The waitlist CTA
reuses `WaitlistForm.tsx` via `WaitlistCTA.astro`. Screenshots live in
`public/screenshots/` named `[platform]-[error-slug]-step-[n].png` (see that
dir's README). The `@astrojs/sitemap` integration auto-includes every
generated `/fix/` route in `sitemap-index.xml`.

### Content schema

Defined in `src/content.config.ts` (collection `errors`, glob loader over
`src/content/errors/*.json`). Per entry:

| Field | Type | Notes |
|---|---|---|
| `platform` | enum `lindy` \| `n8n` \| `relay` | Closed set. |
| `errorMessage` | string | The **verbatim** text users see. Rendered monospace at top. Never paraphrase or invent. |
| `slug` | string | URL slug → `/fix/<platform>/<slug>/`. Lowercase, hyphenated. |
| `plainLanguageMeaning` | string | 1–2 sentences, no jargon. Drives the callout box **and** the meta description. |
| `causes[]` | `{title, explanation, frequency}` | `frequency` ∈ `common`\|`sometimes`\|`rare`; rendered ranked common→rare. |
| `fixSteps[]` | `{step, instruction, screenshot?}` | `step` is a positive int; `screenshot` is an optional `/screenshots/...` path. |
| `whyItHappens` | string? | Optional; renders as an expandable `<details>`. |
| `relatedErrors[]` | string[] (slugs) | Plain slugs, NOT `reference()` — unknown slugs are silently dropped so half-filled states don't break the build. |
| `verified` | boolean (default `false`) | **The gate.** `false` → page shows a red "unverified draft" banner, emits `noindex`, and gets a "draft" pill in indexes. |
| `confidence` | enum `high`\|`medium`\|`low`? | Self-rated; helps prioritize verification. |
| `verificationNote` | string? | What a human must confirm live; shown in the draft banner. |
| `sources[]` | string[] (URLs) | Provenance; rendered in the page footer. |
| `lastVerified` | date? | Set when you flip `verified: true`. |

### Adding a new error page

1. Create one JSON file in `src/content/errors/` (name it anything;
   `<platform>-<slug>.json` by convention — the filename is for humans, the
   route is built from the `platform` + `slug` fields).
2. Fill in `platform`, the verbatim `errorMessage`, a URL `slug`, a 1–2
   sentence `plainLanguageMeaning`, `causes[]`, numbered `fixSteps[]`, and
   optionally `whyItHappens`, `relatedErrors`, `sources`.
3. Leave `verified: false` while drafting. The page auto-builds at
   `/fix/<platform>/<slug>/`, shows in the indexes with a "draft" pill, and
   stays `noindex` so it can't rank prematurely.
4. Add step screenshots to `public/screenshots/` as
   `[platform]-[error-slug]-step-[n].png` and reference each from the matching
   step's `screenshot` field.
5. When you've confirmed the content against the **live** platform, set
   `verified: true` and add `lastVerified`. That removes the banner + `noindex`
   and emits the page's JSON-LD.

**Do not invent error text, causes, or fixes.** Draft from real sources, record
them in `sources`, and verify before flipping `verified`.
...
SEO baseline (`public/robots.txt`, plus the `@astrojs/sitemap`-generated
`sitemap-index.xml`), deploy config, and dev tooling (`vitest`) are
pre-scaffolded so you can ship today.
